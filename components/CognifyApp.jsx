...
function getDeckValidationError(deck) {
...
export default function CognifyApp() {
  ...
  const [lastCorrect, setLastCorrect]     = useState(true) // for animation triggering
...
CRITICAL: Detect the language of the study material and write ALL content (questions, answers, explanations) in that same language. Do not translate into English.`
      ...
      for (let attempt = 1; attempt <= GENERATION_RETRIES; attempt++) {
        ...
        const candidateDeck = parsed?.flashcards
        const validationError = getDeckValidationError(candidateDeck)
        if (!validationError) {
          validDeck = candidateDeck
          break
        }
  ...
  const startQuiz = (deck) => {
    const deckToStart = deck || cards
    const validationError = getDeckValidationError(deckToStart)
    if (validationError) {
      setSetupError(`Bug blocked: ${validationError}`)
      goTo('main')
      return
    }
    scoreRef.current = 0
  ...
  const submitAnswer = () => {
    ...
    if (setsEqual(selected, correctSet)) { 
      ...
      setWrong(w => [...w, { card: cards[idx], selected: [...selected] }])
      setStreak(0)
  ...
  const replaySession = (s) => {
    const validationError = getDeckValidationError(s?.cards)
    if (validationError) {
      setSetupError(`Saved session is invalid: ${validationError}`)
      return
    }
    setCards(s.cards)
  ...
  return (
    <>
      ...
      {screen === 'quiz'    && <QuizScreen cards={cards} idx={idx} score={score} answered={answered} selected={selected} wrong={wrong} streak={streak} lastCorrect={lastCorrect} onPick={pick} onSubmit={submitAnswer} onNext={nextCard} onQuit={() => goTo('main')} />}
